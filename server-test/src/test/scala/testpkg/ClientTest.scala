...
object ClientTest extends AbstractServerTest {
  ...
  private def testArg = s"--sbt-base-directory=$testPath"
  private def clientArgs(args: String*) = Array(testArg) ++ args
  ...
  private def complete(completionString: String, fish: Boolean = false): Seq[String] = {
    val cps = new CachingPrintStream
    SimpleClient.complete(
      Array(testArg) ++ (if (fish) Array("--fish") else Nil) :+ s"--completions=sbtc $completionString",
      false,
      tabs,
      cps
    )
