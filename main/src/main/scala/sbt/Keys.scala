...
object Keys {
  ...
  val updateHash = taskKey[Int]("Gets the potentially cached update report").withRank(Invisible)
  val updateIsCached = taskKey[Boolean]("True if the update has been cached")
  val updateFiles = taskKey[Seq[java.nio.file.Path]]("All of the files used by the update report").withRank(Invisible)
  val updateShouldForce = taskKey[Boolean]("Determines whether an update should be forced").withRank(Invisible)
  ...
  val transitiveUpdateIsCached = taskKey[Boolean]("Indicates whether the transitive update reports are cached.").withRank(Invisible)
