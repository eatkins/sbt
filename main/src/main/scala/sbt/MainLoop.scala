...
import sbt.BasicCommandStrings.{ SetTerminal, StashOnFailure }
  ...
  def processCommand(exec: Exec, state: State): State = {
    ...
    try {
      def process(): State = {
        ...
        if (exec.execId.fold(true)(!_.startsWith(BasicCommands.networkExecPrefix)) &&
            !exec.commandLine.startsWith(BasicCommands.networkExecPrefix)) {
