    ...
    Command(ContinuousExecutePrefix, continuousBriefHelp, continuousDetail)(continuousParser) {
      case (s, (initialCount, commands)) =>
        val (s1, channel) = s.currentCommand.flatMap(_.source.map(_.channelName)) match {
          case Some(c) => s -> c
          case None    => StandardMain.exchange.run(s) -> "console0"
        }
...
//      StateTransform(_ => newState)
  ...
  private def aggregate(
    ...
    val nextInputEvent: WatchExecutor => Option[Watch.Action] = {
      ...
      parseInputEvents(parser, altParser, state, channel.terminal, logger, poll)
    }
...
private[sbt] object ContinuousCommands {
  ...
  private[sbt] val runWatch = BasicCommands.networkExecPrefix + "runWatch"
  private[sbt] val preWatch = BasicCommands.networkExecPrefix + "preWatch"
  private[sbt] val postWatch = BasicCommands.networkExecPrefix + "postWatch"
  private[sbt] val stopWatch = BasicCommands.networkExecPrefix + "stopWatch"
  private[sbt] val failWatch = BasicCommands.networkExecPrefix + "failWatch"
  private[sbt] val waitWatch = BasicCommands.networkExecPrefix + "waitWatch"
  ...
  private[this] val runWatchCommand = watchCommand(runWatch) { (channel, state) =>
    watchStates.get(channel) match {
      ...
      case cs =>
        ...
        val post = FailureWall :: PopOnFailure :: s"$postWatch $channel" :: waitWatch :: Nil
