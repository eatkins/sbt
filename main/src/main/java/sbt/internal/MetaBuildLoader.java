...
public final class MetaBuildLoader extends URLClassLoader {
  ...
  public static MetaBuildLoader makeLoader(final AppProvider appProvider) throws IOException {
    // TODO remove jline without version from pattern once forked jline is published
    final Pattern pattern =
        Pattern.compile("(test-interface-[0-9.]+|jline-[0-9.]+.*|jansi-[0-9.]+|jline)\\.jar");
...

    {
      ...
      for (final File file : cp) {
        ...
        if (!name.equals("jline-2.14.6.jar") && pattern.matcher(name).find()) {
