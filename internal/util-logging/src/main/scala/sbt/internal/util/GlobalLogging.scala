...
final case class GlobalLogBacking(file: File, last: Option[File], newBackingFile: () => File) {
  ...
  def shift(newFile: File): GlobalLogBacking = GlobalLogBacking(newFile, Some(file), newBackingFile)

  /** Shifts the current backing file to `last` and sets the current backing to a new temporary file generated by `newBackingFile`. */
  def shiftNew(): GlobalLogBacking = shift(newBackingFile())
   ...
   */
  def unshift: GlobalLogBacking = GlobalLogBacking(last getOrElse file, None, newBackingFile)
